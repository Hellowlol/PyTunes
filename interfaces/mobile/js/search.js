...
function showDetails(data) {
    ...
    if (data.attr.coverurl) {
        url = WEBDIR + 'search/thumb?url=' + data.attr.coverurl + '&w=200&h=300';
        modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    } else if (data.attr.rageid) {
        url = WEBDIR + 'search/thumb?url=rageid' + data.attr.rageid + '&w=200&h=300';
        modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    }
    ...
    */
    ...
    if (data.attr.backdropurl) {
        url = WEBDIR + 'search/thumb?url=' + data.attr.backdropurl + '&w=675&h=400&o=20';
        $('.modal-fanart').css({
