...
function getCategories() {
    $.ajax({
        url: WEBDIR + 'nzbsearch/getcategories',
        type: 'get',
        dataType: 'json',
...
function search(query, catid) {
    ...
    $.ajax({
        url: WEBDIR + 'nzbsearch/search?q=' + query + '&cat=' + catid,
        type: 'get',
        dataType: 'json',
        beforeSend: function () {
...
function showDetails(data) {
    ...
    if (data.attr.coverurl) {
        url = WEBDIR + 'nzbsearch/thumb?url=' + data.attr.coverurl + '&w=200&h=300';
        modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    } else if (data.attr.rageid) {
        url = WEBDIR + 'nzbsearch/thumb?url=rageid' + data.attr.rageid + '&w=200&h=300';
        modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    }
    ...
    */
    ...
    if (data.attr.backdropurl) {
        url = WEBDIR + 'nzbsearch/thumb?url=' + data.attr.backdropurl + '&w=675&h=400&o=20';
        $('.modal-fanart').css({
...
function showDetails(data) {
    ...
    var modalImage = '';
    if (data.attr["coverurl"]) {
        var url = WEBDIR + 'nzbsearch/thumb?url='+data.attr['coverurl']+'&w=200&h=300';
        var modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    } else if (data.attr["rageid"]) {
        var url = WEBDIR + 'nzbsearch/thumb?url=rageid'+data.attr['rageid']+'&w=200&h=300';
        var modalImage = $('<div>').addClass('thumbnail pull-left');
        modalImage.append($('<img>').attr('src', url));
    }
    ...
    */
    ...
    if (data.attr['backdropurl']) {
        var url = WEBDIR + 'nzbsearch/thumb?url='+data.attr['backdropurl']+'&w=675&h=400&o=20';
        $('.modal-fanart').css({
            'background' : '#ffffff url('+url+') top center no-repeat',
            'background-size' : '100%'
        });
